...
#print(X_tot)
y_tot = np.load('H.npy').tolist()[0:10000]
...
def test_rnn (x1, y_test, plot_min, plot_max):
    ...
    y_test_diags = [np.diagonal(i.reshape(6,6)) for i in y_test]    
    y_pred_diags = [np.diagonal(i.reshape(6,6)) for i in y_pred]
    y0 = np.asarray(y_tot[0])
    eigenvals, eigenvecs = np.linalg.eig(y0.reshape(6,6))
...
#    #np.savetxt(name, y_pred, delimiter=',')
    ...
    for i in range(6):
        ...
        ax.plot([0, X_tot[1000]], [eigenvals[i], eigenvals[i]], label='Actual Energy '+str(i), linewidth=4)
